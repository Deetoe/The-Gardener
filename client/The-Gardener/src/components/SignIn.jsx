...
const SignIn = () => {
  ...
  const validateToken = async (token, discordId) => {
    try {
  ...
  const handleDiscordLogin = async () => {
    ...
    try {
      console.log('Requesting Discord login URL...');
      ...
      let errorMessage = 'Failed to connect to Discord. Please try again later.';
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] p-4">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full text-center">
        <h1 className="text-3xl font-bold mb-4 text-green-600">Sign In to The Gardener</h1>
        <p className="mb-8 text-gray-600">
          Connect with your Discord account to manage your farm from the web!
        </p>
        ...
        <button
          ...
          className={`flex items-center justify-center w-full bg-[#5865F2] text-white font-medium py-3 px-4 rounded-md hover:bg-[#4752C4] transition ${(isLoading || backendStatus === 'offline') ? 'opacity-70 cursor-not-allowed' : ''}`}
        >
          {isLoading ? (
        ...
        <div className="mt-6 text-sm text-gray-500">
          <p>Don't have a farm yet? You'll be able to create one after signing in.</p>
        </div>
